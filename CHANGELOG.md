...
- `CuiInput` and `CuiTextarea` point `aria-describedby` at their own error message. They set `aria-invalid` and rendered a message that nothing referenced, so a screen-reader user was told a field was wrong and never what was wrong with it. A caller's own `aria-describedby` is kept alongside it (#175)
- `CuiFormField` announces `required`. The visible marker is an asterisk that is deliberately `aria-hidden`, and no control in the library has a `required` prop of its own, so required-ness reached assistive tech through no path at all. The label — which `aria-labelledby` points at — now carries a visually hidden "(required)", so it works for every control rather than only the ones that could be taught a new prop (#175)
