...
### Added
- `CuiBanner` gains an `inline` prop: rendered in the flow rather than pinned to a page edge, with no edge rule, a closed box and a radius. For a notice inside a card or panel, where the edge rule has nothing to sit against. It composes with `variant`, so an inline banner can still be solid (#120)
