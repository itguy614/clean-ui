...
### Added
- `titleAs` on `CuiCardHeader`, `CuiModalHeader` and `CuiConfirmDialog`, so the heading level is the consumer's to set — it depends on where the component sits, which the component cannot know. Accepts any heading level or `div` for a decorative title (#129)
