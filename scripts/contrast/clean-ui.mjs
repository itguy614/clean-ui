/**
 * WCAG AA Contrast Checker for @itguy614/clean-ui — Light & Dark mode.
 *
 ...
 *
 * Exports `run()` rather than executing at import time, so scripts/check-contrast.mjs
 * can run this alongside other packages' contrast checks and report one combined
 * summary. This module still does its own per-theme printing exactly as before —
 * only the pass/fail bookkeeping and final summary moved to the generic runner.
 */
import { readFileSync } from "node:fs";
import { resolve, dirname } from "node:path";
import { fileURLToPath } from "node:url";
import { contrastRatio, toHex, grade } from "./color-math.mjs";
...
function parseAllThemes() {
  const themesCss = readFileSync(resolve(__dirname, "../../packages/clean-ui/src/styles/themes.css"), "utf-8");
  ...
  const themeCss = readFileSync(resolve(__dirname, "../../packages/clean-ui/src/styles/theme.css"), "utf-8");
...
/**
 * Runs every clean-ui contrast check, printing the same per-theme detail
 * this script always has, and returns the aggregate result for
 * scripts/check-contrast.mjs's combined summary.
 */
export function run() {
  ...
  console.log("║    Text: 4.5:1  |  Large text: 3:1  |  Non-text (borders/UI): 3:1  |  Light + Dark mode    ║");
  console.log("╚══════════════════════════════════════════════════════════════════════════════════════════════╝\n");
  ...
  function check(mode, theme, label, fg, bg, threshold, info = false) {
    ...
    console.log(`│    ${s}  ${rStr}  ${label.padEnd(48)} ${(fg || "?").padEnd(9)} on ${bg || "?"}`);
  }
  ...
  for (const theme of themes) {
    ...
    // LIGHT MODE
    // ═════════════════════════════════════════════════════════
    console.log(`┌─ ${theme.name} — LIGHT MODE ${"─".repeat(Math.max(0, 71 - theme.name.length))}┐`);
    ...
    // DARK MODE
    // ═════════════════════════════════════════════════════════
    console.log(`┌─ ${theme.name} — DARK MODE ${"─".repeat(Math.max(0, 72 - theme.name.length))}┐`);
  ...
  return { failures, infoCount, totalChecks };
}
