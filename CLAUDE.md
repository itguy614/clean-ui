...
## Critical Gotchas
- **A Vite `define` constant used in library source must be replicated by every app that aliases the library to source.** `src/version.ts` reads `__CUI_VERSION__`, replaced at build time by `packages/clean-ui/vite.config.ts`/`vitest.config.ts` — but since `apps/docs` (and any future docs app) aliases `@itguy614/clean-ui` to workspace *source* instead of `dist`, that file gets bundled through the *consuming app's own* Vite config, not clean-ui's. Forgetting the matching `define` there doesn't fail the build or `vue-tsc` (an undeclared global identifier is only a runtime `ReferenceError`, not a build-time or type error) — it fails silently until something actually imports the barrel in a browser, crashing the whole module before anything mounts (a blank page, no build/test signal at all). `config/workspace-aliases.ts`'s `cleanUiVersionDefine()` is the fix; any app using `workspaceAliases()` must also spread it into its own `define`. This class of bug is exactly why `pnpm build`/`pnpm test` passing isn't sufficient for a docs-site change — load it in a browser.
